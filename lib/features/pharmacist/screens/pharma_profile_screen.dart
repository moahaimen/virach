// Rewritten version of PharmacistSingleProfilePage using abstracted widgets and services
...
class PharmacistSingleProfilePage extends StatefulWidget {
  const PharmacistSingleProfilePage({super.key});

  @override
  State<PharmacistSingleProfilePage> createState() => _PharmacistSingleProfilePageState();
}

class _PharmacistSingleProfilePageState extends State<PharmacistSingleProfilePage>
  ...
  Future<void> _loadProfileData() async {
    ...
    try {
      final rawData = await ProfileService.fetchMeData(
        prefsKey: 'pharmacist_profile_data',
  ...
  void _assignMeData(Map<String, dynamic> data) {
    ...
    final details = (data['role']?['details'] ?? {}) as Map;
    ...
    _pharmacyNameCtrl.text = details['pharmacy_name'] ?? '';
    _bioCtrl.text = details['bio'] ?? '';
    _addressCtrl.text = details['address'] ?? '';
    _sentinel = details['sentinel'] ?? false;
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      await SaveProfileService.saveProfile(
        ...
        centerName: _pharmacyNameCtrl.text,
        bio: _bioCtrl.text,
        availabilityTime: '',
        city: _selectedCity,
        district: _selectedDistrict,
        customCenterFields: {'sentinel': _sentinel},
        updateEndpoint: 'pharmacists',
      );
      ScaffoldMessenger.of(context)
              ...
              children: [
                ...
                ListView(
                  ...
                  children: [
                    ...
                    CheckboxListTile(
                      title: const Text('خدمة الحراسة الليلية'),
                      value: _sentinel,
                      onChanged: _isEditMode
                          ? (val) => setState(() => _sentinel = val ?? false)
                          : null,
                    ),
                  ],
                ),
